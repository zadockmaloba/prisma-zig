...
pub const Parser = struct {
    ...
    pub fn parse(self: *Parser) ParseError!Schema {
        ...
        // Second pass: Build inverse relation metadata for array relation loaders
        try self.buildInverseRelations(&schema);
    ...
    /// Build inverse relation metadata for array relation loaders
    /// This creates the reverse mapping so that if User has restaurantId -> Restaurant.id,
    /// we store that Restaurant has an inverse relation to User[]
    fn buildInverseRelations(self: *Parser, schema: *Schema) ParseError!void {
        // Iterate through all models
        for (schema.models.items) |*from_model| {
            // Check each field for @relation attributes
            for (from_model.fields.items) |*field| {
                ...
                for (schema.models.items) |*model| {
                    if (std.mem.eql(u8, model.name, to_model_name)) {
                        found_model = model;
                        break;
                    }
                }
                ...
                if (found_model) |to_model| {
                    ...
                    const inverse = types.InverseRelation{
                        ...
                        .from_field = fields[0].value, // Foreign key field
